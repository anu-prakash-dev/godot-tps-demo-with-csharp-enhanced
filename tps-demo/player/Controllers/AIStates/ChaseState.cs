...
namespace GodotThirdPersonShooterDemoWithCSharp.Player.Controllers.AIStates
{
    public class ChaseState : State
    {
        ...
        {
            if (payload == null || !payload.Contains("playerTargetLastOrigin"))
            {
                StateMachine.TransitionTo("IdleState");
                return;
            }
            _playerTargetLastOrigin = (Vector3)payload["playerTargetLastOrigin"];
            _currentPath = 0;
            _path = _aiController
            .Navigation.GetSimplePath(_aiController.Player.GlobalTransform.origin, _playerTargetLastOrigin);
            _totalOutOfSight = 0;
        ...
        public override void PhysicsProcess(float delta)
        {
            ...
            if (_totalOutOfSight >= 5f)
            {
                StateMachine.TransitionTo("MoveState",
                new Dictionary() {
                    { "destination", _aiController.InitialPosition }
                });
            }
            ...
            {
                _motion = _motion.LinearInterpolate(Vector2.Zero, AIController.MotionInterpolateSpeed * delta);
